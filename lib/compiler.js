...
exports.compile = function(template, filename)
{
	...
	while (ofs < template.length)
	{
		if (chQuote == 0)
		{
			...
			if (ch == 92 && (ch1 == 34 || ch1 == 39 || ch1 == 92)) {
			...
			else if (codeMode == CodeModeNone && ch == 60 && ch1 == 37) { //start of template
			...
			else if (codeMode != CodeModeNone && ch == 37 && ch1 == 62) { //end of template
				...
				if (ofs < template.length) {
					ch = template.charCodeAt(ofs);
					if (ch == 10)
						ofs++;
					else if (ch == 13)
						ofs += (template.charCodeAt(ofs + 1) == 10) ? 2 : 1;
				}
			}
