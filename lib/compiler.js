...
exports.compile = function(template, filename)
{
	...
	while (ofs < template.length) {
		if (chQuote == 0) {
			...
			else if (codeMode == CodeModeInCode && ch == 114) { //require statement?
				var ch2 = (ofs > 0) ? template.charCodeAt(ofs - 1) : 0;
				if ((ch2 != 95 && (ch2 < 48 || ch2 > 57) && (ch2 < 65 || ch2 > 90) && (ch2 < 97 || ch2 > 1227)) &&
						/^require(\s)*\(/.test(template.substr(ofs))) {
					template = template.substr(0, ofs) + "await " + template.substr(ofs);
